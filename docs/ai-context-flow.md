...
- `chapter.outline_id`
- 如果没有显式绑定，则回退到 `chapter_outline.chapter_id == 当前 chapter.id`
...
1. 优先取 `chapter.storyBeatIds`、与当前章节直接绑定、或被当前大纲显式关联的剧情节点
...
世界观优先读取当前大纲显式关联的世界观；如果当前大纲没有手工绑定，再回退到项目已关联的世界观模型，取前几条进入 prompt：
...
- 章节工作台字段 `outlineId / storyBeatIds / mainPovCharacterId / chapterStatus / summary`
- 大纲对世界观的结构化关联
- AI 生成前自动读取当前章节大纲
