# Story 平台级架构升级：Summary First、StoryUnit、MCP/LSP 与多 Session 编排
...
- 世界观、人物、物品、技能、因果、剧情、章节锚点等信息主要靠应用层临时拼接，缺少一个统一的、可被 MCP/LSP 读取的稳定基础单元。
...
- 结构化不是为了让作者填字段，而是为了让 MCP/LSP、编排层、状态机和回放系统有稳定输入。
...
- 仍可维持单部署、单 JAR。
- 但内部必须拆成多模块，不再继续把 Provider、Setting、Generation、MCP/LSP、Story State 全塞在同一层里。
...
- 建立一套统一的 `StoryUnit` 基础单元与 facet 体系，作为 MCP/LSP 的基础读写单位。
- 明确区分并承载：
  - `Summary`
  - `Canon`
  - `State`
  ...
  - `Reveal`
  - `Execution`
- 建立本地 `MCP Server` 与 `Local Story Protocol Server`，把上下文读取、运行时状态、快照与增量变化从 prompt 中剥离出来。
- 把章节写作链路升级为：
  - 章节总导
  - 选择器
  - 写手
...
- 本地 `MCP Server` 与 `LSP Server` 的职责边界、协议和第一批工具清单。
- 多 session 编排架构。
- 镜头级写作工作流。
- 运行时状态与增量状态体系：
  - 背包
...
- 结构化不能消失，只能下沉。
- JSON 字段、facet、patch、snapshot、event 都应成为 MCP/LSP 和编排层的内部协议。
...
- `StoryUnit + Facets` 成为后续对象读写、生成编排和 MCP/LSP 的统一协议基础。
...
- 若 MCP/LSP 被直接交给模型自由探索，而不是由应用编排层先接管，稳定性反而可能下降。
...
- [2026-04-11 Asia/Shanghai] 确认采用 `StoryUnit + Facets` 作为 MCP/LSP 的统一基础读写单位。
